...
class nebulaos extends oceanos{
	...
	function load_menu($menu){
		foreach($menu as $menu_item){
			if($this->allow($menu_item['appname'],"view")){
				echo '<li class="nav-item" data-appname="'.$menu_item['appname'].'">';
					echo '<a class="nav-link has-icon'.(isset($menu_item['submenu'])?" treeview-toggle":"").'" href="#/'.$menu_item['appname'].'/">';
					$this->menu_load_icon($menu_item);
					...
					if(isset($menu_item['submenu'])){
						...
						foreach($menu_item['submenu'] as $sub_item){
							if($this->allow($sub_item['appname'],"view")){
								echo '<li class="nav-item" data-appname="'.$sub_item['appname'].'">';

								echo '<a class="nav-link has-icon" href="#/'.$sub_item['appname'].'/">';
								$this->menu_load_icon($sub_item);
	...
	function menu_load_caption($item){
		global $_SESSION;
		if(is_array($item['name'])){
			if(isset($item['name'][$_SESSION['lang']])){
				return $item['name'][$_SESSION['lang']];
			}else{
				return $item['name']['en'];
			}
