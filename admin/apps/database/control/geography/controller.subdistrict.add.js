		...
		$.ajax({
			...
			success: function(html){
				...
				fn.app.contact.address.auto_district(
					"form[name=form_addsubdistrict] select[name=country]",
					"form[name=form_addsubdistrict] select[name=city]",
					"form[name=form_addsubdistrict] select[name=district]");
				$("form[name=form_addsubdistrict] select[name=country]").val(211).change();
			}	
		});
	};

	fn.app.database.subdistrict.add = function(){
		$.post('apps/database/xhr/geography/action-add-subdistrict.php',$('form[name=form_addsubdistrict]').serialize(),function(response){
			if(response.success){
