		...
		$.ajax({
			...
			success: function(html){
				...
				fn.app.contact.address.auto_city(
					"form[name=form_adddistrict] select[name=country]",
					"form[name=form_adddistrict] select[name=city]");
				$("form[name=form_adddistrict] select[name=country]").val(211).change();
			}	
		});
	};

	fn.app.database.district.add = function(){
		$.post('apps/database/xhr/geography/action-add-district.php',$('form[name=form_adddistrict]').serialize(),function(response){
			if(response.success){
