<?php
	...
	if($dbc->HasRecord("os_accounts","name = '".$_POST['account']."' AND id != ".$_POST['account_id'])){
		echo json_encode(array(
			'success'=>false,
			'msg'=>'Account Name is already exist.'
		));
	}else if($dbc->HasRecord("os_organizations","name = '".$_POST['name']."' AND id != ".$_POST['org_id'])){
		echo json_encode(array(
			'success'=>false,
			'msg'=>'Organization is already exist.'
		));
	}else{
		$data = array(
			'name' => $_POST['name'],
		...
		$dbc->Update("os_accounts",$data,"id=".$_POST['account_id']);
		$account = $dbc->GetRecord("os_accounts","*","id=".$_POST['account_id']);
		$os->save_log(0,$_SESSION['auth']['user_id'],"account-edit",$_POST['account_id'],array("accounts" => $account));
