var fn={
	...
	service : {
		ajax : function(setting,success){
			/*
			if(typeof success != "undefined"){
				var func = function(){}
				$.extend(setting.success,success);
			}
			$.ajax(setting);
			*/
		},
	...
	dialog : {
		confirmbox : function(title,msg,func){
			bootbox.confirm({
				...
				buttons: {
					confirm: {
						label: ln.global.confirm,
						className: 'btn-primary shadow-0'
					},
					cancel: {
						label: ln.global.cancel,
		...
		change_language : function(){
			bootbox.prompt({
				title: ln.auth.msg.dialog_language_title,
				inputType: 'select',
	...
	navigate : function(app,param){
		...
		if(typeof param != "undefined" && param != ''){
			// Parse query string to new path format
			...
			if(view) {
				...
				// Only id without view (rare case)
				path += id;
			}
		}
	...
	noaccess : function(me){
		fn.notify.warnbox(ln.auth.msg.dialog_noaccess_body,ln.auth.msg.dialog_noaccess_title);
	...
	logout : function(){
		bootbox.confirm({
			title: "<i class='fal fa-exclamation-triangle text-warning mr-2'></i>"+ln.auth.msg.dialog_logout_title,
