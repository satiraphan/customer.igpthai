...
App.ajax = (config) => {
	...
	// If url is empty or just "/" then load default
	if (!url || url === '/' || url.trim() === '') {
		let new_url = config.default;
	...
	if (url) {
		...
		if(path.match(/apps\//)){
			// Old format: apps/dashboard/index.php
			appname = url.split('/')[1];
			// Find menu link by data-appname attribute
			const menuItem = menu.querySelector('li[data-appname="' + appname + '"]');
			currentLink = menuItem ? menuItem.querySelector('a') : null;
			
			// Fallback to old method if not found
			if(!currentLink) {
				currentLink = menu.querySelector('a[href="#' + (url.split('?')[0]) + '"]') || 
				             menu.querySelector('a[href*="apps/' + appname + '/"]');
			}
			
			// Update active menu
			currentLink && App.updateMenu(currentLink)
			if (document.querySelector('.main-navbar')) {
				const navbarItem = document.querySelector('.main-navbar li[data-appname="' + appname + '"]');
				const currentLink2 = navbarItem ? navbarItem.querySelector('a') : null;
				currentLink2 && App.updateMenuTop(currentLink2)
			}
			
			$.post("ajax/auth/action-allow.php",{appname:appname},function(json){
				if(json){
					// load url to container
					App.loadURL(url + window.location.search, currentLink, config)
				}else{
					App.loadURL("iface/noaccess.php", currentLink, config)
				}
			},"json");
			
		}else if(url.charAt(0)=='/'){
			// New routing format: /appname/view/id_or_section/
			const parts = url.split('/').filter(p => p); // remove empty strings
			appname = parts[0]; // level1 = app name
			...
			// Fallback to old method if not found
			if(!currentLink) {
				currentLink = menu.querySelector('a[href*="apps/' + appname + '/"]');
			}
			...
			let new_url = "apps/" + appname + "/index.php";
			...
			if(parts.length > 1 && parts[1] !== '') {
				...
				if(parts.length > 2 && parts[2] !== '') {
					...
					if(/^\d+$/.test(level3)) {
						// เป็นตัวเลข = id parameter
						new_url += "&id=" + level3;
					} else {
						// เป็นตัวอักษร = section parameter
						new_url += "&section=" + level3;
					}
				}
			}
			...
			$.post("ajax/auth/action-allow.php",{appname:appname},function(json){
				if(json){
					// load url to container
					App.loadURL(new_url + window.location.search, currentLink, config)
				}else{
					App.loadURL("iface/noaccess.php", currentLink, config)
				}
			},"json");
		...
		// Load default url
		const defaultLink = menu.querySelector('a[href="#' + config.default + '"]');
		if (defaultLink) {
			window.location.hash = defaultLink.getAttribute('href');
		} else {
			// Fallback: use config.default directly
			window.location.hash = '#' + config.default;
		}
	}
